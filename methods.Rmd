...
Assign all data in one cluster s.t. $Z_1^{(0)}=Z_2^{(0)}=\dots=Z_n^{(0)}=1$, and $K^{(0)}=1$. Sample $\mu_{[1]}^{(0)}$  based on posterior of $\mu|X,Z$, where $n_l$ is count for all $Z_l = j$, detail discussed in appendix A.

$$
P(\mu_{[1]}^{(0)}|X_l,\; where \; Z_l^{(0)}=1) \sim \mathcal{N}( \frac{\frac{\sum x_{l}}{\sigma^2}+\frac{\mu_0}{\sigma_0^2}}{\frac{n_1^{(0)}}{\sigma^2}+\frac{1}{\sigma_0^2}}, [\frac{n_1^{(0)}}{\sigma^2}+\frac{1}{\sigma_0^2}]^{-1})
$$
...
$$
P(\mu_{[k]}^{(t+1)}|X_l,\; where \; Z_l^{(t+1)}=1) \sim \mathcal{N}( \frac{\frac{\sum x_{l}}{\sigma^2}+\frac{\mu_0}{\sigma_0^2}}{\frac{n_k^{(t+1)}}{\sigma^2}+\frac{1}{\sigma_0^2}}, [\frac{n_k^{(t+1)}}{\sigma^2}+\frac{1}{\sigma_0^2}]^{-1})
$$
